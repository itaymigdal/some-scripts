...
script_banner = """
  ...
  ________________     _______  ___   __   _____                                 
 /_  __/ ____/ __ \   / ___/\ \/ / | / /  / ___/_________ _____  ____  ___  _____
  / / / /   / /_/ /   \__ \  \  /  |/ /   \__ \/ ___/ __ `/ __ \/ __ \/ _ \/ ___/
 / / / /___/ ____/   ___/ /  / / /|  /   ___/ / /__/ /_/ / / / / / / /  __/ /    
/_/  \____/_/       /____/  /_/_/ |_/   /____/\___/\__,_/_/ /_/_/ /_/\___/_/     
...
def scan_port(target, port, verbose):
    ...
    syn_ack = 0x12
    rst_ack = 0x14
    response = sr1(IP(dst=target) / TCP(sport=srcport, dport=port, flags=syn), timeout=1, verbose=0)
    if response is None:
        if verbose:
            print("[-] [{}:{}] is filtered or host is down.".format(target, port))
        return "F"
    try:
        response_flags = response.getlayer(TCP).flags
        if response_flags == syn_ack:
            if verbose:
                print("[+] [{}:{}] is open (flags = SYN + ACK).".format(target, port))
            return "O"
        else:
            if response_flags == rst_ack:
                if verbose:
                    print("[-] [{}:{}] is closed (flags= RST + ACK).".format(target, port))
                return "C"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TCP SYN scanner ", formatter_class=argparse.RawTextHelpFormatter,
                                     epilog="----------------------------------------------------------------------")
    parser.add_argument("-address", metavar="<ADDRESS>", type=str, required=True,
                        help="The target address <ip> / <hostname> / <domain>")
    parser.add_argument("-ports", metavar="<PORTS>", type=str, default="80,21,22,23",
