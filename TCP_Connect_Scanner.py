...
script_banner = """ 
  ...
  ________________     ______                            __     _____                                 
 /_  __/ ____/ __ \   / ____/___  ____  ____  ___  _____/ /_   / ___/_________ _____  ____  ___  _____
  / / / /   / /_/ /  / /   / __ \/ __ \/ __ \/ _ \/ ___/ __/   \__ \/ ___/ __ `/ __ \/ __ \/ _ \/ ___/
 / / / /___/ ____/  / /___/ /_/ / / / / / / /  __/ /__/ /_    ___/ / /__/ /_/ / / / / / / /  __/ /    
/_/  \____/_/       \____/\____/_/ /_/_/ /_/\___/\___/\__/   /____/\___/\__,_/_/ /_/_/ /_/\___/_/     
...
commonports = """
               80, 23, 443, 21, 3389, 110, 445, 139, 143, 53, 135, 3306, 8080, 22, 6550, 6600,
...
def http_banner(connsock):
    # try to grab banner by http request
    target = connsock.getpeername()[0]
    message = bytes('HEAD / HTTP/1.1\r\nHost:{}\r\n\r\n'.format(target), "utf-8")
    try:
        connsock.send(message)
        res = connsock.recv(4096)
        return res
    except:
        return None


def tcp_banner(connsock):
    try:
        # get banner
        res = connsock.recv(4096)
    except:
        # try to grab banner with http function
        res = http_banner(connsock)
    if not res:
        # could not grab the banner
        print("[-] Banner is not available\n")
        return
    try:
        # try to decode banner from bits split lines
        banner = res.decode("utf-8").splitlines()
        print("[+] Banner:")
        # split banner lines in order to <tab> before banner lines
        for line in banner:
            # sometimes http servers send headers as html too
            if not line.startswith("<"):
                # print banner lines
                print("\t   ", line)
        print("")
    except:
        # could not get banner propely
        print("[-] Banner is not available\n")


def connscan(target_host, target_port, omitbanner, closeport):
    try:
        # create socket: AF_INET for IPV4, SOCK_STREAM for TCP
        connsock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        # try to connect to target port
        connsock.connect((target_host, target_port))
        # port is open
        print("[+] Target: {} :: port {} is open ".format(target_host, target_port))
        if not omitbanner:
            tcp_banner(connsock)
        else:
            print("")
...
def portscan(target_host, target_ports, omitbanner, closeport):
    try:
        # resolve IP address if domain/hostname supplied
        target_ip = socket.gethostbyname(target_host)
    except:
        print("[-] Error: Unknown host\n")
        exit(0)
    try:
        # try to resolve for hostname address
        target_domain = socket.gethostbyaddr(target_ip)
        print("\n[*] scanning: ", target_ip, " ({}) ...\n".format(target_domain[0]))
    except:
        print("\n[*] scanning: ", target_ip, "...\n")
    # set default timeout
    socket.setdefaulttimeout(10)
    # call connscan for each port
    for port in target_ports:
        connscan(target_ip, int(port), omitbanner, closeport)
    print("[*] scan finished. \n")


def main():
    # parse the command line arguments
    parser = argparse.ArgumentParser(description="smart TCP Port scanner & Banner grabber",
                                     formatter_class=argparse.RawTextHelpFormatter,
                                     epilog="---------------------------------------------------------------------")
    parser.add_argument("-address", metavar="<ADDRESS>", type=str, required=True,
                        help="The target address <ip> / <hostname> / <domain>")
    parser.add_argument("-ports", metavar="<PORTS>", type=str, required=False, default="80,21,22,23",
                        help="The ports to scan \nDefault = <80,21,22,23> \n<*> - 1000 most common TCP ports")
    parser.add_argument("-ob", action="store_true", help="Omit banner grabbing")
    parser.add_argument("-cp", action="store_true", help="Don't print closed ports")
    ...
    if args.ports == "*":
        # if argument <-p *> supplied (scan all 1000 common ports)
        ports = commonports
    else:
        # else ports are those whom supplied
        ports = args.ports
    # split ports
    ports = ports.split(',')
    # call the portscan function
    portscan(target_host=ipaddress, target_ports=ports, omitbanner=omitbanner, closeport=closeport)
