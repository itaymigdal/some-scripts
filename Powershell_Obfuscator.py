...
def remove_newlines_whitespaces(input_data):
    print("[i] Removing newlines...")
    output_data = input_data
    # replace: "| \n" --> "|" (when input is pipe and then newline)
    pipe_newline_pattern = re.compile(r"[|]\s*\n")
    output_data = re.sub(pipe_newline_pattern, "|", output_data)
    # replace: ") \n {" --> "){" (function declaration)
    function_declaration_pattern = re.compile(r"\)\s*\n\s*\{")
    output_data = re.sub(function_declaration_pattern, "){", output_data)
    # replace: "\n" --> ";" (regular newlines)
    output_data = output_data.replace("\n", ";")
    # replace: "  ;  ;  " --> ";"
    whitespaces_and_semicolons_pattern = re.compile(r"[\s;]*;[\s;]*")
    output_data = re.sub(whitespaces_and_semicolons_pattern, ";", output_data)
    # replace: "   " --> " "
    whitespaces_pattern = re.compile(r"\s+")
    output_data = re.sub(whitespaces_pattern, " ", output_data)
    return output_data
...
def add_comments(input_data):
    ...
    for char in input_data:
        if char == ";":
            output_data += get_random_comment() + ";" + get_random_comment()
        else:
            output_data += char
    return output_data
...
def main():
    ...
    obfuscation_group.add_argument("-rn", help="Remove newlines and whitespaces", action="store_true")
    ...
    more_options_group.add_argument("-co", help="Make CMD oneliner to invoke powershell encoded command",
                                    action="store_true")
    ...
    if args.A:
        args.rc = args.rn = args.rv = args.rf = args.ml = args.ac = args.er = True
    if args.rc:
        data = remove_comments(data)
    if args.rn:
        data = remove_newlines_whitespaces(data)
    if args.rv:
